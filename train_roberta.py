...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        self.examples = [convert_examples_to_features(x) for x in data]
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs):
        bar = tqdm(train_dataloader, total=len(train_dataloader), ncols=160, ascii=' >=')
        tr_num = 0
        train_loss = 0
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] > best_f1:
                        ...
                        torch.save(model_to_save.state_dict(), os.path.join(args.output_dir, 'model.bin'))
