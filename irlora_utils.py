...
def get_my_model(model, model_fp, blocksize2=256, tau_range=0.1, tau_n=51):
    ...
    model.model = _replace_with_ours_lora_4bit_linear(model.model, model_fp=model_fp.model, blocksize2=blocksize2, tau_range=tau_range, tau_n=tau_n)
    return model
...
class IRQLoraLinear4bit(bnb.nn.Linear4bit, LoraLayer):
    def __init__(
        ...
        # self.lora_default_C_scale = torch.nn.Parameter(torch.zeros([1], dtype=self.lora_A.default.weight.dtype).to(self.base_layer.weight.device), requires_grad=True)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 在使用get_peft_model方法得到对应的Lora模型后进入此方法，对已经封装好的Lora层进行更改
        # IRQLoraLinear4bit被递归使用于大模型的每一层
        # 1.此处反量化模型本体参数 得到模型权重fp_B
        # with torch.no_grad():
        ...
        if self.disable_adapters or self.active_adapter[0] not in self.lora_A.keys():
            ...
            if not torch.is_autocast_enabled():
                ...
                if self.iii is None:
                    ...
                    # + self.lora_default_A_scale * x.reshape([_ for _ in x.shape[:-1]] + [self.lora_A[self.active_adapter[0]].out_features] + [-1]).mean(dim=-1)
                    # print("=====================>>> iii is None <<<=========================")
                else:
                    # print(self.iii.shape)
                    # print(x.shape)
                    # print("add bert++++++++++++++++++")
                    # print("jkgkgkhg")
                    self.iii = self.iii.to(self.base_layer.compute_dtype)
                    x = self.lora_A[self.active_adapter[0]](self.lora_dropout[self.active_adapter[0]](x)) + self.iii.reshape([_ for _ in self.iii.shape[:-1]] + [self.lora_A[self.active_adapter[0]].out_features] + [-1]).mean(dim=-1)
                        # self.lora_default_C_scale * self.iii.reshape([_ for _ in self.iii.shape[:-1]] + [self.lora_A[self.active_adapter[0]].out_features] + [-1]).mean(dim=-1)
                        # self.iii.reshape([_ for _ in self.iii.shape[:-1]] + [self.lora_A[self.active_adapter[0]].out_features] + [-1]).mean(dim=-1)
                    # + self.lora_default_A_scale * x.reshape([_ for _ in x.shape[:-1]] + [self.lora_A[self.active_adapter[0]].out_features] + [-1]).mean(dim=-1) + \
